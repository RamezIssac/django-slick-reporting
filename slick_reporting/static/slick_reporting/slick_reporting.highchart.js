...
(function ($) {
            ...
            try {
                ...
                let highchart_object = {
                    ...
                    yAxis: {
                        ...
                        opposite: $.slick_reporting.highcharts.defaults.rtl,
                    },
                    xAxis: {
                        // title: {text: chartxAxisTitle},
                        labels: {enabled: true},
                        reversed: $.slick_reporting.highcharts.defaults.rtl,
                    },
                    tooltip: {
                        useHTML: Highcharts.hasBidiBug
                    },
        ...
        function get_time_series_data(response, chartOptions) {
            ...
            if (!chartOptions.plot_total) {
                ...
                Object.keys(data_sources).forEach(function (series_cols, index) {
                    ...
                    series.push({
                        'name': $.slick_reporting.highcharts.defaults.messages.total,
                        'data': data
                    })
                })
            }
        ...
        function get_crosstab_data(response, chartOptions) {
            ...
            let series = []
            let data_sources = {};
            let col_dict = $.slick_reporting.dataComprehension.dataArrayToObject(response.columns, 'name')
            chartOptions.data_source.forEach(function (elem, key) {
                data_sources[elem] = [];
                response.columns.forEach(function (col, key) {
                    if (col.computation_field === elem) {
        ...
        function displayChart(data, $elem, chart_id) {
            ...
            let chart = $elem;
            let chartObject = $.slick_reporting.dataComprehension.getObjFromArray(data.chart_settings, 'id', chart_id, true);
